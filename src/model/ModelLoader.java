...
public class ModelLoader {
	...
	public TopicModel loadModel(String modelName, String domain,
		...
		try {
			...
			double[][][] twDistOne = loadTopicWordOne(domain, modelDirectory);
			
			double[][] dwdist = loadDocumentTopic(modelDirectory
					+ File.separator + domain + ModelPrinter.documentTopicDistSuff);
			
			if (modelName.equals("LDA")) {
				//double[][] dwdist, double[][][] twDistOne, double[][] twdist
				return new LDAvb(corpus, param, dwdist, twDistOne, twdist);
			} else if (modelName.equals("LTM")) {
				return new LTMvb(corpus, param, dwdist, twDistOne, twdist);
			} else {
				ExceptionUtility
	 ...
	 * Description: 
	 * @author: LiuFeng
	 * @date: 2017年3月24日 下午4:35:37
	 * @param domain
	 * @return
	 */
	public double[][] loadDocumentTopic(String filepath) {
	...
	public double[][][] loadTopicWordOne(String domain, String modelDirectory) {
		...
		for (int d = 0; d < D; ++d) {
			...
			String[] wordsWithTopics = line.trim().split("$$$$$");
			int N = wordsWithTopics.length;
			z[d] = new double[N][];
			for (int n = 0; n < N; ++n) {
				
				String[] words = wordsWithTopics[n].trim().split(" ");
				z[d][n] = new double[words.length];
				for (int k = 0; k < words.length; k++) {
					z[d][n][k] = Double.parseDouble(words[k].replace("$$$$$", ""));
				}				
			}
		}
		return z;
